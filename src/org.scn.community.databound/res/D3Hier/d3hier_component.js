...
sap.designstudio.sdk.Component.subclass("org.scn.community.databound.jpd3hier", function() {
	 ...
	 * If you use the getters be sure to use a () afterwards. Else you get the function defined here ;)
	 */
	this.autoProperties = {
		rsltSet : null,	/* stores Informations about the Datasource Metadata and data values*/
		useDummyData : true,
		showText : true,
		showWarnings : true,
		showRootBubble : true,
		alerterUsePercentage : true,
		alerterHighValuesAreGood : true,
		dimHierarchy : "NONE",
		kfBubbleSize : "NONE",
		selectedNode : "",
		kfColBub : "NONE",
		whichTextShown : "text_value",
		BubbleSizeMaxSize : 10,
		BubbleRootXTranslation : 0,
		BubbleSizeBoderSize : 3,
		AlerterColorGreenVal : 30,
		AlerterColorYellowVal : 10,
		//AlerterColorRedVal : 2,
		alertColorRed : "red",
		alertColorYellow : "yellow",
		alertColorGreen : "green"
	};
 ...
 ************************************ */
	...
	this.update = function (source) {
		...
		nodeEnter.append("svg:text")
			.attr("class", function(d) {return "circleText1ExpCol_" + that.owner.sId;})
			.attr("x", function(d) { return -9;/*d.children || d._children ? -10 : 10;*/ })
			.attr("dy", ".30em")
			.on("click", function(d) { 
		...
		nodeEnter.append("svg:text")
			.attr("class", function(d) {return "circleText2ExpCol_" + that.owner.sId;})
		...
		var link = that.variables.vis.selectAll("path.link")
			.data(that.variables.tree.links(nodes), function(d) { return d.target.id; });
		...
		link.enter().insert("svg:path", "g")
			...
			.attr("d", function(d) {
				...
				return that.variables.diagonal({source: o, target: o});
			})
			.transition()
			.duration(duration)
 ...
 ************************************ */
    	...
    	if (that.rsltSet() && !that.useDummyData()) {
            ...
            */
            ...
            for (var i = 0; i < numTuples; i++) {               //Loop numTuples times
            	...
            	var localKey = "", localText = "", localType = "", localNodestate = "", localLevel = "", localname = "", localSize = -1, 
            	hierDimInstance = -1, kfDimInstance = -1;
                ...
                try {
                	...
                	if (kfDimInstance === that.variables.bubbleSizeKeyFigId) {
	                	...
	                	if(this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].hasOwnProperty("key")) {localKey = this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].key;};
	                	if(this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].hasOwnProperty("type")) {localType = this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].type;};
	                	if(this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].hasOwnProperty("nodeState")) {localNodestate = this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].nodeState;};
	                	if(this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].hasOwnProperty("level")) {localLevel = this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].level;};
	                	if(this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].hasOwnProperty("text")) {localText = this.rsltSet().dimensions[that.variables.tupelColumnHier].members[hierDimInstance].text; 
			...
			tempJsonData = { "name": "World", "size": 2311, "key": "HIERARCHY_NODE/0HIER_NODE/WELT", "text": "WORLD", "type": "HIERARCHY_NODE", "nodeState": "COLLAPSED", "level": 1,
							"children": [
					             {
				            	 	"name": "North America", "size": 814, "key": "HIERARCHY_NODE/0HIER_NODE/NORDAMERIKA", "text": "NORDAMERIKA", "type": "HIERARCHY_NODE", "nodeState": "COLLAPSED", "level": 2,
				            	 	"children": [
		            	 	             {"name": "JFK", "size" : 394, "key": "JFK", "text": "JFK", "level": 3},
		            	 	             {"name": "SFO", "size" : 420, "key": "SFO", "text": "SFO", "level": 3}
	            	 	             ]
					             }
					             ,{
					            	 "name": "Asia", "size": 498, "key": "HIERARCHY_NODE/0HIER_NODE/ASIEN", "text": "ASIA", "type": "HIERARCHY_NODE", "nodeState": "COLLAPSED", "level": 2,
					            	 "children": [
				            	        {"name": "NRT", "size" : 112, "key": "NRT", "text": "NRT", "level": 3},
				            	        {"name": "SIN", "size" : 278, "key": "SIN", "text": "SIN", "level": 3},
				            	        {"name": "TYO", "size" : 108, "key": "TYO", "text": "TYO", "level": 3}
				            	     ] 
					             }
					             ,{"name": "Europe", "size": 1000, "key": "HIERARCHY_NODE/0HIER_NODE/EUROPA", "text": "EUROPE", "type": "HIERARCHY_NODE", "nodeState": "COLLAPSED", "level": 2,
					            	 "children": [
			            	              {"name": "FCO", "size" : 268, "key": "FCO", "text": "FCO", "level": 3},
			            	              {"name": "TXL", "size" : 46, "key": "TXL", "text": "TXL", "level": 3},
			            	              {"name": "FRA", "size" : 686, "key": "FRA", "text": "FRA", "level": 3}
		            	              ]
					             }
				           ]
					};
	...
	this.indexDimension = function(idx){
		...
		if(!this.rsltSet()) return null;
		if(this.rsltSet().dimensions.length>=idx) return this.rsltSet().dimensions[idx].key;
		return null;
	};
	...
	this.dimensionIndexByKey = function(key){
		...
		for(var i=0;i<this.rsltSet().dimensions.length;i++){
			if(this.rsltSet().dimensions[i].key==key) return i;
		}
		return null;
	};
	...
	this.dimensionKFIndex = function(){
		if (this.rsltSet()) { // otherwise there is an exception with no data connection ..
			if (this.rsltSet().dimensions) { // check if there is a dimensions property ...
				for(var i=0;i<this.rsltSet().dimensions.length;i++){
					if(this.rsltSet().dimensions[i].hasOwnProperty("containsMeasures")) return i;
				}
				return null;
