			...
			this.afterUpdate = function() {
				try{
					...
					for(var i=0;i<cols.length;i++){
						...
						if(col.orderable != undefined) newDef.orderable = col.orderable;
						if(col.searchable != undefined) newDef.searchable = col.searchable;
						if(col.title != undefined) newDef.title = col.title;
						// if(col.sortDirection != undefined && col.sortDirection != "none") newDef.orderSequence = [col.sortDirection]
						if(col.type != undefined && col.type != "auto") newDef.type = [col.type]
						columnDefs.push(newDef);
					}
					var dom = "lfrtip";
					var search = {};
					if(this.searchTerm()) search.search = this.searchTerm();
