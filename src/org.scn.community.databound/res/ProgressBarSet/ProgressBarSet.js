/**
 * CC BY-NC-SA License
 * Progress Bar Set by Mike Howles is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.
...
(function() {
	 ...
	 sdkReqs(["require"], function(require) {
		 ...
		 * ProgressBarSet
		 */
		 ProgressBarSet.prototype = org_scn_community_databound_Base;
		 ProgressBarSet.prototype.constructor = ProgressBarSet;
		 ProgressBarSet.prototype.toString = function(){
	     ...
	     function ProgressBarSet() {
	    	...
	    	this.componentInfo.title = "Progress Bar Set";
	    	this.componentInfo.description = "Progress Bar Set is a simple databound example of how to create a set of progress bars.  This is an enhancement of the non databound Progress Bar Set found in the Prototypes category.";
	    	...
	    	// Translate orientation based on horizontal/vertical
	    	...
	    	this.drawBars = function(){
	    		...
	    		var that = this;
	    		var data = [];
	    		var pm = this.progressMeasure() || "{}";
	    		var progressObj = jQuery.parseJSON(pm);
	    		var measureKey = this.determineMeasureName(progressObj);
	    		var measureIndex = -1;
	    		for(var i=0;i<this.flatData.columnHeadersKeys.length;i++){
	    			if(this.flatData.columnHeadersKeys[i] == measureKey) measureIndex = i;
	    		}
	    		this._barConfig = [];
	    		for(var i=0;i<this.flatData.values.length;i++){
	    			this._barConfig.push({
	    				title : this.flatData.rowHeaders[i],
	    				number : this.flatData.values[i][measureIndex],
	    				fillColor : "auto"
	    			});
	    		}
		...
		sap.designstudio.sdk.Component.subclass(ownComponentName, ProgressBarSet);	// End of SDK
