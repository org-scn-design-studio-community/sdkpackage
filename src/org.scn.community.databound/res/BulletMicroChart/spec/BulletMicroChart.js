...
BulletMicroChart = {
	...
	prepareCustomData: function (owner) {
		...
		if(org_scn_community_databound.hasData (dataCellListThreshold1)) {
			dataCellListThreshold1 = org_scn_community_databound.flatten (dataCellListThreshold1,options);
			useDataCellListThreshold1 = true;
		}
		...
		for(rowI=0;rowI<counterI;rowI++){
			...
			if(!that.getUseThresholds()) {
				if(useDataCellListThreshold1) thresholdsJson.push({value:  dataCellListThreshold1.values[rowI][0], color: "Good"});
