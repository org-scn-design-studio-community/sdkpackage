...
Breadcrumbs = {
	...
	initAsync: function (owner) {
		...
		that.addStyleClass("scn-pack-Breadcrumbs");
	...
	prepareCustomData: function (owner) {
		...
		for(rowI=0;rowI<counterI;rowI++){
			...
			l_Links = org_scn_community_unified.loopObjectArray(that, l_Links, rowI);
			...
			customData[l_Links.name] = l_Links.json;
		...
		if(org_scn_community_unified.adjustBreadcrumbsModel) {
			that._specialDataModel = org_scn_community_unified.adjustBreadcrumbsModel(that._specialDataModel, that);
		}
...
myComponentData.instance = Breadcrumbs;
