 ...
 * Create ColorBuilder Extension
 */
sap.ui.core.Control.extend("org.scn.community.aps.ColorPicker2",{
	metadata : {                             
        properties : {
        	value : {
				type : "string",
	 ...
	 contrastingColor : function(color) { return (this.luma(color) >= 165) ? '#000000' : '#FFFFFF'; },
	 hexToR:function(h) {return parseInt((this.cutHex(h)).substring(0,2),16);},
	 hexToG:function(h) {return parseInt((this.cutHex(h)).substring(2,4),16);},
	 hexToB:function(h) {return parseInt((this.cutHex(h)).substring(4,6),16);},
	 cutHex:function(h) {return (h.charAt(0)=="#") ? h.substring(1,7):h;},
	...
	onAfterRendering : function(){
		this.getAggregation("_layout").getContent()[0]
			.setValue(this.getValue())
			//$.css({"background-color" : this.getValue()})
			//.css({"color" : this.contrastingColor(this.RGBtoHEX(this.getValue()))});
	},
	destroy : function(){
		if(this._tp) this._tp.destroy();
	},
	...
	init : function(){
		try{
		var that = this;
		var _pickerButton = new sap.ui.commons.Button({
			//text : " ",
			//width : "75px",
			icon : "data:image/png;base64,",
			press : function(oControlEvent){
				var pop = new sap.ui.ux3.ToolPopup({
					modal : true,
					opener : this,
					close : function(oControlEvent){
						this.destroyContent();
				...
				var oldColor = that.getValue();
				pop.destroyContent();
				pop.destroyButtons();
				try{
				var cs = "rgba(" + 
					that.hexToR((that.getValue())?that.getValue():"#FFFFFF")+","+
					that.hexToG((that.getValue())?that.getValue():"#FFFFFF")+","+
					that.hexToB((that.getValue())?that.getValue():"#FFFFFF")+","+
					1 /*alpha*/ + ")";
					pop.addContent(
						new org.scn.community.aps.ColorPickerUI5({
							showAlpha : false,
							...
							change : function(oControlEvent){
								var color = oControlEvent.getParameter("hex");
								if(!color) color = that.getValue();
								that.setValue(color);
								alert(JSON.stringify(that.getModel().getData()));
							}
						})
					);
				}catch(e){
				...
				pop.addButton(new sap.ui.commons.Button({
					text : "Ok",
					press : function(oControlEvent){
						pop.close(false);
						pop.fireClose();
		...
		var _colorTextBox = new sap.ui.commons.TextField({
			tooltip : "Color Hex Code",
			width : "75px",
			/*change : function(){
				var v = this.getValue();
		...
		_colorTextBox.bindProperty("value","value");
		var layout = new sap.ui.layout.HorizontalLayout({});
		layout.addContent(_colorTextBox);
		layout.addContent(_pickerButton);
		this.setAggregation("_layout", layout);
	}catch(e){
		alert("Color Picker init problem:\n\n" + e);
	}
	},
	renderer : {
		render : function(oRm, oControl){
			...
			//layout.getContent()[0].bindProperty("value","value");
