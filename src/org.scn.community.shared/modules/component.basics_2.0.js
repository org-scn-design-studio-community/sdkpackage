...
define(["../os/numberformat/numberformatter"], function () {
	// Declare ONCE are return as a Require Module
	var org_scn_community_basics = {
		version : "3.0",
		resizeContentAbsoluteLayout : function (parent, mainObject, callback) {
			if (parent._oContentPlaced != true) {
				...
				if (!org_scn_community_basics.onWindowResizeInserted) {
					org_scn_community_basics.addEvent(window, "resize", org_scn_community_basics.onWindowResize);
					org_scn_community_basics.onWindowResizeInserted = true;
				}
		...
		findParentContainer : function (parent) {
			var myParent = parent;
		...
		determineOwnSize : function (parent, resizeTrigger) {
			...
			if (orgParent.oComponentProperties) {
				...
				if (orgParent._oldContainerWidth != orgParent._containerWidth) {
					sizeChanged = true;
					if (resizeTrigger)
						orgParent._oldContainerWidth = orgParent._containerWidth;
				}
				if (orgParent._oldContainerHeight != orgParent._containerHeight) {
					sizeChanged = true;
					if (resizeTrigger)
						orgParent._oldContainerHeight = orgParent._containerHeight;
				}
			}
		...
		onWindowResizeExecutor : function () {
	...
	org_scn_community_basics.getFormattedValue = function (value, locale, numberOfDecimal) {
		...
		var number = jQuery.formatNumber(value, {
				format : formatString,
				locale : locale
			});
