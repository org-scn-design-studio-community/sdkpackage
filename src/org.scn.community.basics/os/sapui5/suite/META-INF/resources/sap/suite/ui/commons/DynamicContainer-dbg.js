...
sap.suite.ui.commons.DynamicContainer.prototype.init = function() {
	...
	this._oPauseIcon = new sap.ui.core.Icon(this.getId() + "-pause-icon",{
		src : "sap-icon://media-pause",
	}); 
	this._oPauseIcon.addStyleClass("sapSuiteDCIcon");	
};
...
//	jQuery.event.special.tap.tapholdThreshold = this.getPauseTriggerTime();
//	
//	this.$().bind("taphold", function(oE) {
//		that._toggleAnimation();
//		that._bPreventEndEvent = true;
//	});
};

sap.suite.ui.commons.DynamicContainer.prototype._handleSwipeLeft = function(oE) {
	if(sap.ui.getCore().getConfiguration().getRTL()){
		oE.data.parent._scrollToPrevTile(true);
	} else {
		oE.data.parent._scrollToNextTile(true);
	}
	
	clearTimeout(this._sTimerId);
	oE.data.parent._bPreventEndEvent = true;
};

sap.suite.ui.commons.DynamicContainer.prototype._handleSwipeRight = function(oE) {
	if(sap.ui.getCore().getConfiguration().getRTL()){
		oE.data.parent._scrollToNextTile(true);
	} else {
		oE.data.parent._scrollToPrevTile(true);
...
//
//
//sap.suite.ui.commons.DynamicContainer.prototype.onmouseover = function(oE) {
...
//};
//
//sap.suite.ui.commons.DynamicContainer.prototype.onmouseout = function(oE) {
//	if(jQuery.device.is.desktop) {
//		if(this._bAnimationPause)
//			this._toggleAnimation();
//		oE.stopPropagation();
...
sap.suite.ui.commons.DynamicContainer.prototype._toggleAnimation = function() {
	if (this.getTiles().length > 1) {
		if (this._bAnimationPause) {
			...
			this._oPauseIcon.$().hide();
		} else {
			this._stopAnimation();
			
			this.$().bind("swipeleft", {parent: this}, this._handleSwipeLeft);
			this.$().bind("swiperight", {parent: this},this._handleSwipeRight);
			
			this._oPauseIcon.$().css("display", "block");
			var that = this;
			setTimeout(function() {
				that._oPauseIcon.$().hide(1000);
			}, this.getPauseTriggerTime());
		}
	}
