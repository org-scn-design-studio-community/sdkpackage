...
sap.suite.ui.commons.ComparisonChartRenderer.render = function (oRm, oControl) {
    ...
    oRm.write(">");
    	...
    	oRm.writeStyles();
    	oRm.write(">");
    		for (var i = 0; i < oControl.getData().length; i++) {
    			var oBar = oControl.getData()[i];
    			oRm.write(oBar.getTitle() + " " + (oBar.getDisplayValue() ? oBar.getDisplayValue() : oBar.getValue()) + oControl.getScale() + " " + oBar.getColor() + "\n");
    		}
        oRm.write("</span>");
    oRm.write("</div>");
};
