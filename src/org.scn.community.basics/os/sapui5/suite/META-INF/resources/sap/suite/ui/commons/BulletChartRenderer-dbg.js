...
sap.suite.ui.commons.BulletChartRenderer.render = function (oRm, oControl) {
    ...
    var sOrientationOpposite = bRtl ? "left" : "right";
    ...
    oRm.writeAttribute("aria-labelledby", sDescId);
    ...
    oRm.write(">");
        ...
        oRm.write("<span");
    	oRm.writeAttribute("id", sDescId);
	    oRm.writeAttribute("aria-hidden", "true");
    	oRm.addStyle("display", "none");
    	oRm.writeStyles();
    	oRm.write(">");
    		if (bIsActualSet) {
    			if (sActualValueLabel) {
    				oRm.write(sActualValueLabel);
    			} else {
    				oRm.write(oControl.getActual().getValue());
    			}
    			
    			oRm.write(oControl.getScale() + " " + oControl.getActual().getColor());
    		}
    		if (oControl._isTargetValueSet) {
	    		oRm.write(" Target ");
	    		if (sTargetValueLabel) {
	    			oRm.write(sTargetValueLabel);
	    		} else {
	    			oRm.write(oControl.getTargetValue());
	    		}
	    		oRm.write(oControl.getScale());
    		}

    		if (oControl._isForecastValueSet && "Actual"==sMode) {
    			oRm.write(" Forecast ");
    			oRm.write(oControl.getForecastValue() + oControl.getScale());
    		}
    		
    		for (var i = 0; i < oControl.getThresholds().length; i++) {
    			var oThreshold = oControl.getThresholds()[i];
    			oRm.write(" Threshold " + oThreshold.getValue() + oControl.getScale() + " " + oThreshold.getColor());
    		}
        oRm.write("</span>");
    oRm.write("</div>");
};
