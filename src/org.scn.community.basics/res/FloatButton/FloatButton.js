...
FloatButton = {
	...
	initAsync: function (owner) {
		...
		that.addStyleClass("scn-pack-FloatButton");
	    ...
	    that.addEventDelegate({
	        onAfterRendering: function () {
				that._oFloatButton.dummy = true;
				org_scn_community_basics.resizeContentAbsoluteLayout(that, that._oFloatButton, that.onResize);
	...
	afterPrepare: function (owner) {
		...
		var width = that.oComponentProperties.width + "px";
		var height = that.oComponentProperties.height + "px";

		that._oFloatButton.setWidth(width);
		that._oFloatButton.setHeight(height);
		...
		if(lElementsToRender != null && lElementsToRender != undefined && lElementsToRender != ""){
			...
			for (var i = 0; i < lElementsToRenderArray.length; i++) {
				...
				if(that._oElements[element.key] == undefined) {
					var lNewElement = that._createElement(that, i, element.key, element.text, element.image, element.parentKey, element.selected, element.leaf, element.width);
					...
					if(element.width) {
						currentElement.setWidth(element.width + "px");
					}
			...
			if(lElement != undefined) {
				...
				if(lElement._Placed != true) {
					...
					if(allKeysAsOfNow.indexOf("|"+lElementKey+"|") == -1) {
						...
						if(parentKey == "ROOT") {
							that._oFloatButton.removeButton(lElement);
	...
	onResize: function(width, height, parent) {
		...
		parent._oFloatButton.setWidth(width + "px");
		parent._oFloatButton.setHeight(height + "px");
	},
	...
	_initComponent : function() {
		...
		that._oFloatButton = new sap.ui.commons.Panel({
			width : "100%",
		...
		that.addContent(that._oFloatButton);
	},
	...
	_destroyAll : function (owner) {
		...
		that._oFloatButton.removeAllButtons();
		that._oFloatButton.destroyButtons();
	...
	_addRoot : function(owner, iElement) {
		...
		iElement.setLayoutData(new sap.ui.layout.ResponsiveFlowLayoutData({
			weight : 1
		}))
		
		that._oFloatButton.addContent(iElement);
	},
	...
	_prepareImage: function (owner, iImageUrl) {
		...
		if(iImageUrl.indexOf("http") == 0) {
			...
			iImageUrl = org_scn_community_basics.getRepositoryImageUrlPrefix(that, that.getDDefaultImage(), iImageUrl, "FloatButton.png");	
		}
		return iImageUrl;
	},

	_createElement: function (owner, index, iElementKey, iElementText, iImageUrl, iParentKey, iSelected, isLeaf, iWidth) {
		var that = owner;
...
//%INIT-START%
myComponentData.instance = FloatButton;
jQuery.sap.require("sap.ui.layout.ResponsiveFlowLayout");
sap.ui.layout.ResponsiveFlowLayout.extend(myComponentData.fullComponentName, myComponentData.instance);
});