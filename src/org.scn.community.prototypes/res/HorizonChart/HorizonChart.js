...
sap.designstudio.sdk.Component.subclass("org.scn.community.prototypes.horizonChart", function() {
...
this.afterUpdate = function() {
	...
	if (that.trellisDimension() !== "None" && that.isEpoch() == "true") {var horizonTrellisMember = horizon_data.dimensions[2].members}
	else if (that.trellisDimension() !== "None" && that.isEpoch() == "false"){var horizonTrellisMember = horizon_data.dimensions[3].members};
	...
	var trellis_data = [];  
	trellisSet = d3.nest()
	      .key(function(d) { return d.trellis; })
	      .entries(final_data);
	...
	// We need this at 2 levels, once to identify the right trellis array, and again inside to enable mousemove/click functions
		...
		var mean = trellisMeasure.reduce(function(p, v) { return p + v; }, 0) / trellisMeasure.length;
	  ...
	  var overlayRect =	trellisEnter.append('rect')
	  	 ...
	  	 chartContainers[0].forEach(function(container, i) {
	  		 ...
	  		 var textEnter = textLabel.enter()
	  		 	...
	  		 	.text(function(d){
	  		 		if (that.trellisDimension() == "None") {return ""}
	  		 		else {return d.key}
	  		 	});
	    	...
	    	var overlaySelect = d3.select("rect#" + horizonProps[0].gId + ".overlay"),
	    		...
	    		overlayMouse = overlaySelect.on("mouseover", function() { focusSelect.style("display", null); })
	  	  		.on("mouseout", function() { focusSelect.style("display", "none"); })
...
function click(d) {
    ...
    if (that.trellisDimension() !== "None") {that.selectedNode(d.trellis);}
